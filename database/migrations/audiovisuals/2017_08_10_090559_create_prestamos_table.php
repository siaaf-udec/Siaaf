...
class CreatePrestamosTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('audiovisuals')->create('TBL_Prestamos', function (Blueprint $table) {
            ...
            $table->integer('PRT_FK_Funcionario_id')->unsigned()->nullable(); //funcionario
            $table->integer('PRT_FK_Kits_id')->unsigned()->nullable(); //->id kits
            $table->date('PRT_Fecha_Inicio'); //reserva y prestamo
            $table->date('PRT_Fecha_Fin'); //reserva y prestamo
            $table->string('PRT_Observacion_Entrega');
            $table->string('PRT_Observacion_Recibe');
            $table->integer('PRT_FK_Estado')->unsigned()->nullable(); // pendiente aprobado o rechazado
            $table->integer('PRT_FK_Tipo_Solicitud')->unsigned()->nullable(); // prestamo o reserva
            $table->integer('PRT_FK_Administrador_Entrega_id')->unsigned()->nullable(); //administradores
            $table->integer('PRT_FK_Administrador_Recibe_id')->unsigned()->nullable(); //administradores
