<div class="col-md-12">
    @component('themes.bootstrap.elements.portlets.portlet', ['icon' => 'fa fa-tasks', 'title' => 'Etapa de monitoreo y control:'])
        <h4 style="margin-top: 0px;">Editar proceso: Monitorear y controlar el trabajo del proyecto.</h4>
    <br>
    <br>
    <div class="row">
        <div class="col-md-12">
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        columns = [{
            ...
            {
                defaultContent: '<a href="javascript:;" class="btn btn-sm btn-success verEtapas"  title="Cambiar el estado de este objetivo" ><i class="fa fa-exchange"></i></a>',
                data: 'action',
                name: 'Acciones',
                title: 'Acciones',
                orderable: false,
        ...
        table.on('click', '.verEtapas', function(e) {
            ...
            var route = '{{ route('calidadpcs.procesosCalidad.storeProceso16') }}';
                    var type = 'POST';
                    var async = async ||false;
                    var formData = new FormData();
                    formData.append('id_Objetivo', dataTable.PK_CO_Id_Objetivo);
                    formData.append('Estado', dataTable.CO_Estado);
                    $.ajax({
                        url: route,
                        headers: {
        ...
        $(".guardarProceso").on('click', function(e) {
            e.preventDefault();
                var route = '{{ route('calidadpcs.procesosCalidad.storeProceso16_1') }}';
                    var type = 'POST';
                    var async = async ||false;
                    var formData = new FormData();
