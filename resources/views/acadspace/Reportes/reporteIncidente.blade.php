...
<main>
    <div id="details" class="clearfix">
        ...
        <div id="invoice">
            ...
            <div class="date">Incidentes ocurridos en {{$obtEspacio[0]->ESP_Nombre_Espacio}} </div>
            <div class="date">Fecha del reporte: {{$date}}</div>
            <div class="date">Hora del reporte: {{$time}}</div>
            <div>
                <div>
                    <a class="noPrint" href="{{ route('espacios.academicos.report.descargarRepInc',[$fech1,$fech2,$labNum])}}">
    ...
    <div align="center">
        <h1>Incidentes ocurridos en {{$obtEspacio[0]->ESP_Nombre_Espacio}}</h1>
    ...
    <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
            <th class="no">#</th>
            <th class="unit"><b>DOCUMENTO DEL IMPLICADO</b></th>
            <th class="unit"><b>ESPACIO </b></th>
            <th class="unit"><b>ARTICULO </b></th>
            <th class="unit"><b>DESCRIPCION </b></th>
        </tr>
        </thead>
        @for($i=0;$i<$num;$i++)
            <tr>
            <td class="no">{{$cont++}}</td>
            <td class="unit">{{$obtIncidentes[$i]->FK_INC_Id_User}}</td>
            <td class="unit">{{$obtEspacio[0]->ESP_Nombre_Espacio}}</td>
            <td class="unit">{{$obtIncidentes[$i]->articulo->ART_Codigo}}</td>
            <td class="unit">{{$obtIncidentes[$i]->INC_Descripcion}}</td>
            </tr>
        @endfor
    </table>
    <br><br>
