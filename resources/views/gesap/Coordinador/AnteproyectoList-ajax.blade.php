...
<script>
    jQuery(document).ready(function () {
        ...
        columns = [
            ...
            {data: 'PK_NPRY_IdMinr008', name: "PK_NPRY_IdMinr008", "visible": false},
            ...
            {data: 'NPRY_FechaR', name: "NPRY_FechaR", className: 'none', searchable: true},
            {data: 'NPRY_FechaL', name: "NPRY_FechaL", className: 'none', searchable: true},
            {data: 'NPRY_Estado', searchable: true, name: 'Estado'},
            {
                data: 'radicacion.RDCN_Min', name: "RDCN_Min", className: 'none',
                render: function (data, type, full, meta) {
                    return '<a class="document" href="{{ route('download.documento') }}/' + data + '">DESCARGAR MIN</a>';
                }
            },
            {
                data: 'radicacion.RDCN_Requerimientos',
                name: "RDCN_Requerimientos",
                className: 'none',
                searchable: true,
                render: function (data, type, full, meta) {
                    if (data == "NO FILE") {
                ...
                render: function (data, type, full, meta) {
                    if (full.NPRY_Estado == "<span class='label label-sm label-success'>APROBADO<\/span>") {
                        if (full.proyecto == null) {
                            return '@permission("MODIFY_PROJECT_GESAP")<a href="#" title="Modificar proyecto" class="btn btn-simple btn-warning btn-icon edit" data-toggle="modal" data-target="#"><i class="icon-pencil"></i></a>@endpermission @permission("ASSIGN_TEACHER_GESAP")<a href="#" title="Asignar proyecto" class="btn btn-simple btn-success btn-icon assign"><i class="icon-users"></i></a>@endpermission @permission("DELETE_PROJECT_GESAP")<a href="javascript:;" title="Eliminar proyecto" class="btn btn-simple btn-danger btn-icon remove"><i class="icon-trash"></i></a>@endpermission';
                        } else {
                            if (full.proyecto.PRYT_Estado == "TERMINADO") {
                                return '<span class="label label-sm label-success">Proyecto Terminado</span>';
                            } else {
                                return '@permission("ASSIGN_TEACHER_GESAP")<a href="#" title="Asignar Profesor" class="btn btn-simple btn-success btn-icon assign"><i class="icon-users"></i></a>@endpermission<span class="label label-sm label-success">Proyecto en curso</span>';
                            }
                        }
                    } else {
                        return '@permission("MODIFY_PROJECT_GESAP")<a href="#" title="Modificar Proyecto" class="btn btn-simple btn-warning btn-icon edit" data-toggle="modal" data-target="#"><i class="icon-pencil"></i></a>@endpermission @permission("ASSIGN_TEACHER_GESAP")<a href="#" title="Asignar profesor" class="btn btn-simple btn-success btn-icon assign"><i class="icon-users"></i></a>@endpermission @permission("DELETE_PROJECT_GESAP")<a href="javascript:;" class="btn btn-simple btn-danger btn-icon remove"><i class="icon-trash"></i></a>@endpermission';
                    }
                },
            }
        ],
