                        ...
                        <div class="col-md-12">
                            <form @submit.prevent="editRequest" id="financial-form-request" role="form" :action="portlet.route" method="put">
                                <div class="form-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <vue-select2 :label="portlet.select.action.label"
                                                         v-model.number="action"
                                                         :value="action"
                                                         :options="actions"
                                                         :attributes="portlet.select.action.attributes"
                                                         name="action">
                                            </vue-select2>
                                        </div>
                                    </div>
...
<script>
    ...
    export default {
            ...
            return {
                portlet: {
                    ...
                    select: {
                        ...
                        action: {
                            label: Lang.get('validation.attributes.action').capitalize(),
                            attributes: {
                                disabled: false,
                                required: true,
                                class: null,
                ...
                teachers: [],
                actions: [
                    {id: 1, text: Lang.get('financial.add-sub.actions.addition') },
                    {id: 2, text: Lang.get('financial.add-sub.actions.subtract') }
                ],
                current: {
                    subject_fk: null,
                    teacher_fk: null,
        ...
        methods: {
            ...
            getSource: function () {
                axios.get( route('financial.api.add-sub.edit', {id: $('#app').data('source') }) )
                    .then( (response) => {
                        this.current = response.data;
                    })
                    .then( () => { this.action = this.current.subject_action; })
