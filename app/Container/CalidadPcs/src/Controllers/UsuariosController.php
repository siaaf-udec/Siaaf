...
class UsuariosController extends UsersUdecController
{
    ...
    {
        return view('calidadpcs.usuarios.tablaUsuarios');
    ...
    public function indexAjax(Request $request)
    {
        if ($request->ajax() && $request->isMethod('GET')) {
            return view('calidadpcs.usuarios.ajaxTablaUsuarios');
        }
    ...
    public function create(Request $request)
    {
        if ($request->ajax() && $request->isMethod('GET')) {
            ...
            return view('calidadpcs.usuarios.registroUsuario',
                [
    ...
    public function create2(Request $request)
    {
        if ($request->ajax() && $request->isMethod('GET')) {
            $listaDependencias = Dependencias::all();
            return view('calidadpcs.usuarios.registroUsuario2',
                [
                    'listaDependencias' => $listaDependencias,
                ]);
        }
        ...
        if ($request->ajax() && $request->isMethod('POST')) {
            ...
            if (is_null($verificiarusercedula) && empty($verificiaruser)){
                    ...
                    if (is_null($verificarUserUdec) ) {
                        ...
                        $perfil=Dependencias::where('PK_CD_Id_Dependencia', $request['FK_CU_Id_Dependencia'])->first();
                ...
                if (empty($verificiaruser)) {
                    ...
                    Usuarios::create([
                        ...
                        'FK_CU_Id_Dependencia' => $request['FK_CU_Id_Dependencia'],
                        'FK_CU_Id_Estado' => $request['FK_CU_Id_Estado'],
                    ]);
    ...
    public function edit(Request $request, $id)
    {
        if ($request->ajax() && $request->isMethod('GET')) {
           ...
           //$infoUsuario=Usuarios::all()->where('CU_Cedula',$id)->get();
            ...
            return view('calidadpcs.usuarios.editarUsuario',
                [
    ...
    public function destroy(Request $request, $id)
    {
        if ($request->ajax() && $request->isMethod('DELETE')) {
            ...
            $infoMoto = Usuarios::with('relacionUsuariosProyectos')->where('PK_CU_Codigo', $id)->get();

            if (null === $infoMoto[0]['relacionUsuariosProyectos']) {
                Usuarios::destroy($id);
                return AjaxResponse::success(
                    '¡Bien hecho!',
            ...
            for ($i = 0; $i < sizeof($infoMoto[0]['relacionUsuariosProyectos']); $i++) {
                Proyectos::destroy($infoMoto[0]['relacionUsuariosProyectos'][$i]['PK_CP_Id_proyecto']);
            }
        ...
        if ($request->ajax() && $request->isMethod('GET')) {
            ...
            return DataTables::of($user2)
                ...
                ->addColumn('CU_Perfil', function ($user2) {
                    $perfil=Dependencias::where('PK_CD_Id_Dependencia', $user2->FK_CU_Id_Dependencia)->first();
